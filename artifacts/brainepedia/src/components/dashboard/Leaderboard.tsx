...
import { Trophy, Medal, Zap, Crown, ChevronUp } from "lucide-react";
...
function Avatar({
  ...
  return (
    <div
      className={`${cls} bg-gradient-to-br from-[#7C3AED] to-[#00D2FF] ${glow || ""}`}
    >
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[0, 1, 2].map(i => (
          <div
            key={i}
            className="rounded-xl border border-white/6 bg-white/3 p-4 flex flex-col items-center gap-3 animate-pulse"
          >
...
export function Leaderboard({ topUsers, currentUser, loading }: LeaderboardProps) {
  ...
  return (
    <div className="space-y-4">
            ...
            return (
              <motion.div
                ...
                className={`flex items-center gap-3 px-4 py-3 transition-colors
                  ${isMe
                    ? "bg-[#FFD700]/6 border-l-2 border-l-[#FFD700]/50"
                    : "hover:bg-white/2"
                  }`}
              >
                ...
                <p className={`flex-1 text-sm font-mono truncate ${isMe ? "text-[#FFD700] font-bold" : "text-white/70"}`}>
                  {user.nickName}
                </p>
      ...
      {/* ── Sticky "Your Position" — only when current user is outside top list ── */}
      <AnimatePresence>
        {showYourPosition && (
          <motion.div
